<x-app-layout>
    ...
    <div class="p-4 max-w-xl">
        <div class="bg-white shadow-sm rounded-lg p-4">
            <form method="POST" action="{{ route('menu-items.update', $item) }}" enctype="multipart/form-data" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
                ...
                <div>
                    ...
                    @if(!empty($item->image_path))
                        <img src="{{ $item->image_path }}" alt="{{ $item->name }}" class="mb-2 h-24 w-24 object-cover rounded border" />
                    @endif
