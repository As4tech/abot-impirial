<x-app-layout>
    ...
    <div class="p-4 max-w-2xl">
        <div class="bg-white shadow-sm rounded-lg p-4">
            <form method="POST" action="{{ route('inventory.products.store') }}" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" name="unit" value="{{ old('unit') }}" placeholder="e.g. bottle, crate, kg" class="w-full border rounded px-3 py-2" />
                    @error('unit')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-medium">Initial Stock</label>
                    <input type="number" step="0.001" name="stock_quantity" value="{{ old('stock_quantity', 0) }}" class="w-full border rounded px-3 py-2" />
