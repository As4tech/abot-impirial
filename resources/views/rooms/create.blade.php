<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">New Room</h2>
    </x-slot>

    <div class="p-4 max-w-2xl">
        <form method="POST" action="{{ route('rooms.store') }}" enctype="multipart/form-data" class="space-y-4 bg-white p-6 rounded shadow-sm">
            @csrf

            <div>
                <label class="block text-sm font-medium">Room Number</label>
                <input type="text" name="room_number" value="{{ old('room_number') }}" class="mt-1 w-full border rounded px-3 py-2" required />
            ...
            <div>
                ...
                <select name="room_type_id" class="mt-1 w-full border rounded px-3 py-2" required>
                    ...
                    @foreach($roomTypes as $roomType)
                        <option value="{{ $roomType->id }}" @selected((string) old('room_type_id') === (string) $roomType->id)>{{ $roomType->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="stay_type" class="mt-1 w-full border rounded px-3 py-2" required>
                    <option value="long" @selected(old('stay_type', 'long') === 'long')>Long</option>
                    <option value="short" @selected(old('stay_type') === 'short')>Short</option>
                </select>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="long_price" value="{{ old('long_price') }}" class="mt-1 w-full border rounded px-3 py-2" required />
                @error('long_price')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="block text-sm font-medium">Short Price</label>
                <input type="number" step="0.01" name="short_price" value="{{ old('short_price') }}" class="mt-1 w-full border rounded px-3 py-2" required />
            ...
            <div class="md:col-span-2">
                ...
                <div class="flex flex-wrap gap-4">
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="has_ac" value="1" {{ old('has_ac') ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        <span class="ml-2 text-sm text-gray-600">AC</span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="has_fan" value="1" {{ old('has_fan') ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        <span class="ml-2 text-sm text-gray-600">Fan</span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="has_tv" value="1" {{ old('has_tv') ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        <span class="ml-2 text-sm text-gray-600">TV</span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="has_fridge" value="1" {{ old('has_fridge') ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
