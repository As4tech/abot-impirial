<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Room Types</h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    @can('room-types.create')
                        <div class="mb-4">
                            <a href="{{ route('room-types.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 active:bg-blue-900 focus:outline-none focus:border-blue-900 focus:ring ring-blue-300 disabled:opacity-25 transition ease-in-out duration-150">
                                Add Room Type
                            </a>
                        </div>
                    @endcan
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($roomTypes as $roomType)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900"><x-currency :amount="$roomType->base_price" /></div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">{{ $roomType->capacity }} guests</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">{{ $roomType->rooms_count }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex items-center gap-3">
                                                <a href="{{ route('room-types.show', $roomType) }}" title="View" class="inline-flex items-center text-indigo-600 hover:text-indigo-900">
                                            ...
                                            @can('room-types.edit')
                                                <a href="{{ route('room-types.edit', $roomType) }}" title="Edit" class="inline-flex items-center text-blue-600 hover:text-blue-900">
